...
public class PGPFileProcessor {
    ...
    public boolean encrypt(InputStream keyIn, OutputStream out, ByteArrayOutputStream text) throws Exception {
        PGPUtils.encryptText(out, text, PGPUtils.readPublicKey(keyIn), asciiArmored, integrityCheck);
        out.close();
        keyIn.close();
        return true;
    }

    public boolean decrypt(InputStream in, InputStream keyIn, OutputStream out) throws Exception {
        PGPUtils.decryptText(in, out, keyIn, passphrase.toCharArray());
        in.close();
        out.close();
        keyIn.close();
        return true;
    ...
    public boolean signEncrypt() throws Exception {
        ...
        PGPUtils.signEncryptFile(out, this.getInputFileName(), publicKey, secretKey, this.getPassphrase(),
