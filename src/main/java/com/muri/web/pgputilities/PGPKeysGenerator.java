...
public class PGPKeysGenerator {

    public static class Pair<T, U> {

	public void setFirst(T string) {
	    // TODO Auto-generated method stub

	}

	public void setSecond(U string) {
	    // TODO Auto-generated method stub
	...
	char pass[] = { 'h', 'e', 'l', 'l', 'o' };
	PGPKeyRingGenerator krgen = generateKeyRingGenerator("rauleraul@example.com", pass);

	// Generate public key ring, dump to file.
	ByteArrayOutputStream paux = new ByteArrayOutputStream();
	PGPPublicKeyRing pkr = krgen.generatePublicKeyRing();
	OutputStream pubout = new ArmoredOutputStream(paux);
	pkr.encode(pubout);
	pubout.close();
	System.out.println(new String(paux.toByteArray()));

	// Generate private key, dump to file.
	ByteArrayOutputStream aux = new ByteArrayOutputStream();
	PGPSecretKeyRing skr = krgen.generateSecretKeyRing();
	OutputStream secout = new ArmoredOutputStream(aux);
	skr.encode(secout);
	secout.close();
	System.out.println(new String(aux.toByteArray()));

	// String pukey = b64.encode(pubKey.getEncoded());
	// String prkey = b64.encode(privKey.getEncoded());
	//
	// OutputStream pustream = new FileOutputStream(publicKeyFilename);
	// OutputStream prstream = new FileOutputStream(privateFilename);
	//
	//
	// prstream = new ArmoredOutputStream(prstream);
	// prstream.write(prkey.getBytes());
	// System.out.println("privateKey : " + b64.encode(privKey.getEncoded()));
	// prstream.close();
    }

    public static Pair<String, String> generateKeys(String email, String password) {
	try {
	    Pair<String, String> resp = new Pair<String, String>();

	    PGPKeyRingGenerator krgen = generateKeyRingGenerator(email, password.toCharArray());
	    ...
	    pkr.encode(pubout);
	    pubout.close();
	    resp.setFirst(new String(paux.toByteArray()));
	    ...
	    skr.encode(secout);
	    secout.close();
	    resp.setSecond(new String(aux.toByteArray()));
	    return resp;
	} catch (Exception e) {
	    e.printStackTrace();
	}
